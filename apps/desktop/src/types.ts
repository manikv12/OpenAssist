...
export type MacSyncPeerStatus = {
  ...
  lastSyncedAt?: string;
  lastError?: string;
  lastSyncSummary?: MacSyncSummary;
  hasRoute: boolean;
  inboundLastSeenAt: string | null;
  advice: string | null;
  syncing: boolean;
};
...
export type PlannerDigestLeftover = {
  itemID: string;
  title: string;
  suggestion: "tomorrow" | "backlog" | "keep" | "drop";
  reason: string;
};

export type PlannerDigestFollowUp = {
  title: string;
  reason: string;
  sourceTitle: string;
  when: "tomorrow" | "backlog";
};

export type PlannerDailyDigestResult =
  | {
      ok: true;
      dayID: string;
      nextDayID: string;
...
export type PlannerDailyDigestPlan = {
  dayID: string;
  leftovers: Array<{ itemID: string; action: "tomorrow" | "backlog" | "keep" | "drop" }>;
  followUps: Array<{ title: string; reason: string; sourceTitle: string; when: "tomorrow" | "backlog" }>;
};
