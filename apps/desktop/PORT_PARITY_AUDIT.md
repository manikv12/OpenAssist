...
- Deliver React + Electron app inside current OpenAssist repo.
  - Evidence: `apps/desktop/package.json`, `apps/desktop/README.md`, `apps/desktop/electron/main.ts`, `apps/desktop/electron/preload.ts`, `apps/desktop/electron/openassistBridge.ts`, `apps/desktop/src/App.tsx`, `apps/desktop/src/styles.css`.
  - Result: complete.
...
- Copy provider icons and professional visual styling.
  ...
  - App icon evidence: the OpenAssist icon is stored at `apps/desktop/icon.icns`; `package:mac` uses `--icon=icon`; packaged `Contents/Resources/electron.icns` has the same SHA-256 hash as `icon.icns`, and `CFBundleIconFile` points to `electron.icns`.
...
- React + Electron app lives inside current OpenAssist repo.
  - Evidence: `apps/desktop/package.json`, `apps/desktop/electron/main.ts`, `apps/desktop/src/App.tsx`.
  - Status: covered.
...
- Main chat provider dropdown must match native OpenAssist, not show unrelated external providers.
  ...
  - Electron evidence: `runtimeProviderOptions` in `apps/desktop/src/App.tsx`.
...
- Chat provider selection must use the same selected-thread provider idea as native OpenAssist.
  - Electron evidence: `setThreadProvider` in `apps/desktop/electron/openassistBridge.ts`.
...
- Chat-side notes, instructions, and memory panels should work.
  ...
  - Electron evidence: thread/project notes are loaded from the OpenAssist note stores, can be selected, edited, saved, formatted through a note toolbar, previewed as markdown, and can receive chart markdown through the OpenAssist note layer instead of writing chart folders inside `apps/desktop`.
...
- Provider icons should be copied.
  - Evidence: `apps/desktop/src/assets/provider-marks/*.svg`.
...
- Sidebar show/hide shortcut.
  - Evidence: `CommandOrControl+Shift+Space` in `apps/desktop/electron/main.ts`; renderer handles `openassist:toggle-sidebar-shortcut`.
  - Status: covered.

- macOS menu.
  - Evidence: `installApplicationMenu()` in `apps/desktop/electron/main.ts`.
  - Status: covered.

- Voice-to-text mode.
  - Evidence: Apple Speech helper plus local `whisper-transcribe-helper.swift` and copied macOS `whisper.framework` in `apps/desktop/electron/helpers/`.
...
- Charts must not create chart folders inside Electron folder.
  - Evidence: verifier rejects local chart/chart-store folders under `apps/desktop`.
  - Status: covered.
...
- Handoff artifact: `README.md` is inside `apps/desktop/` with the run/build/package/verify commands and architecture notes.
- Project hygiene: `.gitignore` keeps generated Electron artifacts out of version control, including `node_modules/`, `dist-electron/`, `dist-renderer/`, `out/`, `out-unpacked/`, `.vite/`, logs, and `verification/`.
