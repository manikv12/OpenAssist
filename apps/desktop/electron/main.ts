...
let plannerReminderWindow: BrowserWindow | null = null;
let plannerReminderAutoHideTimer: NodeJS.Timeout | null = null;
...
<style>
  ...
  * {
  ...
  .hud-live-link-host {
    ...
    padding: 1px 6px;
    border-radius: 999px;
    background: color-mix(in srgb, var(--orb-c1, #6f9dff) 24%, transparent);
...
<script>
  ...
  function updateLiveHUDContent(payload, phase, container) {
    ...
    const assistantMessageNode = scope.querySelector(".hud-live-message.is-assistant");
    const assistantMessageTextNode = assistantMessageNode && assistantMessageNode.querySelector(".hud-live-message-text");
    const userMessageNode = scope.querySelector(".hud-live-message.is-user");
    const userMessageTextNode = userMessageNode && userMessageNode.querySelector(".hud-live-message-text");
    ...
    // Clickable link chips: when the assistant's answer or a finished agent
    // task carries URLs (shopping options, docs, results), surface them here
    // so the user can open them in the browser without opening the app.
    ...
    if (linksRow) {
      ...
      if (linksRow.dataset.key !== key) {
        ...
        links.forEach((link) => {
          ...
          chip.type = "button";
          chip.className = "hud-live-link";
          chip.title = link.url;
          const host = document.createElement("span");
          host.className = "hud-live-link-host";
          try {
            host.textContent = new URL(link.url).hostname.replace(/^www\./, "");
          } catch {
            host.textContent = "link";
          }
          const label = document.createElement("span");
          label.className = "hud-live-link-label";
          label.textContent = oneLine(link.label) || host.textContent;
          chip.append(host, label);
          wireLiveHUDButton(chip, "openLink:" + link.url);
          linksRow.appendChild(chip);
        });
      }
      linksRow.classList.toggle("is-visible", links.length > 0);
    }
  }
  ...
  window.updateOpenAssistVoiceHUD = function(payload) {
    ...
    if (status === "processing") {
      ...
      const createLiveMessage = (role, roleLabel) => {
        ...
        const label = document.createElement("span");
        label.className = "hud-live-message-role";
        label.textContent = roleLabel;
        const text = document.createElement("span");
        text.className = "hud-live-message-text";
      ...
      providerNode.className = "hud-live-provider";
      const metaDot = document.createElement("span");
      metaDot.className = "hud-live-meta-dot";
      ...
      work.type = "button";
      work.className = "hud-live-work";
      work.title = "Open agent work";
      work.setAttribute("aria-label", "Open agent work");
...
function shortcutMonitorHelperSourcePath() {
  const candidates = [
    ...
    path.join(openAssistRepoRoot(), "apps", "desktop", "electron", "helpers", "shortcut-monitor-helper.swift")
  ];
...
// The capture frame is always-on-top and (on macOS) visible on every Space,
// so without a guard it keeps floating over whatever the user switches to —
// content it no longer annotates. While the frame is up, a native helper
// streams app-activation / Space-change events (instant); a slow poll is the
// fallback when the helper cannot run. Hide/show use a short opacity fade so
// the frame eases away instead of blinking.
let screenAnalysisCapturedApp: FrontmostApplicationSnapshot | null = null;
let screenAnalysisFrameAutoHidden = false;
let screenAnalysisFrameGuardTimer: NodeJS.Timeout | null = null;
let screenAnalysisFrameMonitorProcess: ReturnType<typeof spawn> | null = null;
let screenAnalysisFrameGuardGeneration = 0;
const screenAnalysisFrameGuardFallbackIntervalMs = 1200;

function appActivationMonitorSourcePath() {
  const candidates = [
    path.join(__dirname, "..", "electron", "helpers", "app-activation-monitor.swift"),
    path.join(process.resourcesPath ?? "", "app", "electron", "helpers", "app-activation-monitor.swift"),
    path.join(openAssistRepoRoot(), "apps", "desktop", "electron", "helpers", "app-activation-monitor.swift")
  ];
  return candidates.find((candidate) => fs.existsSync(candidate));
}

async function ensureAppActivationMonitorHelper() {
  const sourcePath = appActivationMonitorSourcePath();
  if (!sourcePath) throw new Error("App activation monitor helper source was not found.");
  const helperDirectory = path.join(app.getPath("userData"), "helpers", "Open Assist App Activation Monitor");
  const helperPath = path.join(helperDirectory, "app-activation-monitor");
  const sourceStat = fs.statSync(sourcePath);
  const helperStat = fs.existsSync(helperPath) ? fs.statSync(helperPath) : null;
  if (!helperStat || helperStat.mtimeMs < sourceStat.mtimeMs) {
    fs.mkdirSync(helperDirectory, { recursive: true });
    ...
    try {
      ...
      // Ad-hoc is fine during local development.
    }
  }
  return helperPath;
}

// After an auto-hide fade the document sits at opacity 0; any non-guard show
// path (overlay update, eye toggle) must reset it or it reveals an invisible
// window.
function resetScreenAnalysisFrameOpacity(frameWindow: BrowserWindow) {
  void frameWindow.webContents.executeJavaScript(
    "document.documentElement.style.transition='none'; document.documentElement.style.opacity='1';"
  ).catch(() => undefined);
}

function fadeScreenAnalysisFrame(frameWindow: BrowserWindow, visible: boolean) {
  const js = visible
    ? "document.documentElement.style.transition='opacity 160ms ease'; requestAnimationFrame(() => { document.documentElement.style.opacity='1'; });"
    : "document.documentElement.style.transition='opacity 130ms ease'; document.documentElement.style.opacity='0';";
  return frameWindow.webContents.executeJavaScript(js).catch(() => undefined);
}

function autoHideScreenAnalysisFrame(frameWindow: BrowserWindow, reason: string) {
  if (!frameWindow.isVisible() || frameWindow.isDestroyed()) return;
  screenAnalysisFrameAutoHidden = true;
  void fadeScreenAnalysisFrame(frameWindow, false);
  setTimeout(() => {
    if (!frameWindow.isDestroyed() && screenAnalysisFrameAutoHidden) frameWindow.hide();
  }, 140);
  debugLog(`screen analysis frame auto-hidden (${reason})`);
}

function autoShowScreenAnalysisFrame(frameWindow: BrowserWindow) {
  if (frameWindow.isDestroyed() || frameWindow.isVisible()) return;
  screenAnalysisFrameAutoHidden = false;
  // Start transparent while hidden, then fade in once shown; a hidden window
  // still executes JS, only rAF-driven animation needs the window visible.
  void frameWindow.webContents.executeJavaScript(
    "document.documentElement.style.transition='none'; document.documentElement.style.opacity='0';"
  ).catch(() => undefined).then(() => {
    if (frameWindow.isDestroyed() || screenAnalysisFrameAutoHidden) return;
    frameWindow.showInactive();
    frameWindow.moveTop();
    void fadeScreenAnalysisFrame(frameWindow, true);
  });
  debugLog("screen analysis frame restored (captured app is frontmost again)");
}

// Shared decision: called with the pid that just became frontmost (from the
// native event stream) or with a polled snapshot (fallback).
function applyScreenAnalysisFrameGuardDecision(frontmostPid: number, frontmostName = "") {
  const frameWindow = screenAnalysisFrameWindow;
  if (!frameWindow || frameWindow.isDestroyed() || screenAnalysisStatus === "idle") {
    stopScreenAnalysisFrameGuard();
    return;
  }
  // Respect the manual eye toggle; the guard only manages automatic hiding.
  if (!screenAnalysisFrameVisible) return;
  // Our own app coming to front (panel interaction) never hides the frame.
  if (frontmostPid === process.pid) return;
  const captured = screenAnalysisCapturedApp;
  const onCapturedApp = Boolean(captured) && frontmostPid === captured?.pid;
  if (!onCapturedApp) {
    autoHideScreenAnalysisFrame(frameWindow, `frontmost ${frontmostName || frontmostPid} != captured ${captured?.name || "unknown"}`);
  } else if (screenAnalysisFrameAutoHidden) {
    autoShowScreenAnalysisFrame(frameWindow);
  }
}

function stopScreenAnalysisFrameGuard() {
  screenAnalysisFrameGuardGeneration += 1;
  if (screenAnalysisFrameGuardTimer) {
    clearInterval(screenAnalysisFrameGuardTimer);
    screenAnalysisFrameGuardTimer = null;
  }
  const monitor = screenAnalysisFrameMonitorProcess;
  screenAnalysisFrameMonitorProcess = null;
  if (monitor && !monitor.killed) monitor.kill();
}

function startScreenAnalysisFrameGuardFallbackPoll() {
  if (screenAnalysisFrameGuardTimer) return;
  screenAnalysisFrameGuardTimer = setInterval(() => {
    void runScreenAnalysisFrameGuardTick();
  }, screenAnalysisFrameGuardFallbackIntervalMs);
}

function startScreenAnalysisFrameGuard() {
  if (process.platform !== "darwin") return;
  if (screenAnalysisFrameGuardTimer || screenAnalysisFrameMonitorProcess) return;
  const generation = ++screenAnalysisFrameGuardGeneration;
  void (async () => {
    try {
      const helperPath = await ensureAppActivationMonitorHelper();
      if (generation !== screenAnalysisFrameGuardGeneration) return;
      const child = spawn(helperPath, [], { stdio: ["pipe", "pipe", "pipe"] });
      screenAnalysisFrameMonitorProcess = child;
      let buffer = "";
      child.stdout.setEncoding("utf8");
      child.stdout.on("data", (chunk: string) => {
        ...
        while (newline >= 0) {
          ...
          if (!line) continue;
          try {
            const payload = JSON.parse(line) as { event?: string; pid?: number; name?: string };
            if (payload.event === "activate" && typeof payload.pid === "number") {
              applyScreenAnalysisFrameGuardDecision(payload.pid, payload.name || "");
            } else if (payload.event === "spaceChanged") {
              // A Space switch may keep the same app frontmost, so verify with
              // one snapshot after macOS settles instead of guessing.
              setTimeout(() => { void runScreenAnalysisFrameGuardTick(); }, 250);
            }
          } catch {
      ...
      child.on("exit", () => {
        if (screenAnalysisFrameMonitorProcess === child) {
          screenAnalysisFrameMonitorProcess = null;
          // Helper died while a session is active: fall back to polling.
          if (generation === screenAnalysisFrameGuardGeneration && screenAnalysisStatus !== "idle") {
            startScreenAnalysisFrameGuardFallbackPoll();
          }
        }
      });
      child.on("error", () => {
        if (screenAnalysisFrameMonitorProcess === child) screenAnalysisFrameMonitorProcess = null;
        if (generation === screenAnalysisFrameGuardGeneration) startScreenAnalysisFrameGuardFallbackPoll();
      });
    } catch (error) {
      debugLog(`app activation monitor unavailable, falling back to polling: ${error instanceof Error ? error.message : String(error)}`);
      if (generation === screenAnalysisFrameGuardGeneration) startScreenAnalysisFrameGuardFallbackPoll();
    }
  })();
}

async function runScreenAnalysisFrameGuardTick() {
  const frameWindow = screenAnalysisFrameWindow;
  if (!frameWindow || frameWindow.isDestroyed() || screenAnalysisStatus === "idle") {
    stopScreenAnalysisFrameGuard();
    return;
  ...
  if (!screenAnalysisFrameVisible) return;
  // While the user interacts with any of our windows (the analysis panel,
  // the assistant), the frame should stay put.
  if (BrowserWindow.getFocusedWindow()) return;
  const snapshot = await refreshFrontmostApplicationSnapshot();
  if (!snapshot || isOwnApplicationSnapshot(snapshot)) return;
  applyScreenAnalysisFrameGuardDecision(snapshot.pid, snapshot.name);
}
...
async function updateScreenAnalysisOverlay(payload: {
  ...
  if (!frameWindow.isDestroyed()) {
    ...
    if (screenAnalysisFrameVisible) {
      resetScreenAnalysisFrameOpacity(frameWindow);
      if (!frameWindow.isVisible()) frameWindow.showInactive();
      screenAnalysisFrameAutoHidden = false;
      startScreenAnalysisFrameGuard();
    } else {
      frameWindow.hide();
    }
...
function setScreenAnalysisFrameVisible(visible: boolean) {
  ...
  if (visible) {
    resetScreenAnalysisFrameOpacity(screenAnalysisFrameWindow);
    screenAnalysisFrameWindow.showInactive();
    screenAnalysisFrameWindow.moveTop();
    // Manual show re-anchors the frame guard to whatever app is in front
    // now, so the guard does not immediately hide it again.
    screenAnalysisFrameAutoHidden = false;
    if (process.platform === "darwin") {
      void refreshFrontmostApplicationSnapshot().then((snapshot) => {
        if (snapshot && !isOwnApplicationSnapshot(snapshot)) screenAnalysisCapturedApp = snapshot;
      }).catch(() => undefined);
    }
    startScreenAnalysisFrameGuard();
  } else {
...
async function cancelScreenAnalysisPrompt() {
  ...
  screenAnalysisFrameVisible = true;
  stopScreenAnalysisFrameGuard();
  screenAnalysisCapturedApp = null;
  screenAnalysisFrameAutoHidden = false;
  screenSelectionWindow?.close();
...
async function triggerScreenAnalysis() {
  ...
  try {
    ...
    // Remember which app is being captured so the frame guard can hide the
    // border when the user later switches to a different app or Space.
    screenAnalysisFrameAutoHidden = false;
    if (process.platform === "darwin") {
      const capturedApp = await frontmostApplicationSnapshotWithMaxAge(1_500).catch(() => null);
      screenAnalysisCapturedApp = capturedApp && !isOwnApplicationSnapshot(capturedApp) ? capturedApp : null;
    }
...
function createTranscriptHistoryWindow() {
  ...
  // Hide instead of destroy (same as the settings window) so reopening is
  // instant — a destroyed window cold-boots the whole renderer and shows the
  // logo splash every time.
  window.on("close", (event) => {
...
function textInserterHelperSourcePath() {
  const candidates = [
    ...
    path.join(openAssistRepoRoot(), "apps", "desktop", "electron", "helpers", "text-inserter-helper.swift")
  ];
  return candidates.find((candidate) => fs.existsSync(candidate));
}
...
type PlannerReminderDuePayload = {
  dayID: string;
  itemID: string;
  title: string;
  reminderAt: string;
...
function plannerReminderFallbackHTML(payload: PlannerReminderDuePayload) {
  const due = new Date(payload.reminderAt);
  const dueText = Number.isFinite(due.getTime())
    ? due.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })
    : "Now";
  const iconCandidates = [
    path.join(__dirname, "..", "public", "AppLogo.png"),
    path.join(__dirname, "..", "assets", "AppLogo.png"),
    path.join(process.resourcesPath ?? "", "app", "public", "AppLogo.png"),
    path.join(process.resourcesPath ?? "", "app", "assets", "AppLogo.png")
  ];
  const iconPath = iconCandidates.find((candidate) => fs.existsSync(candidate));
  const iconHTML = iconPath
    ? `<img class="app-icon" src="data:image/png;base64,${fs.readFileSync(iconPath).toString("base64")}" alt="" />`
    : `<span class="app-icon app-icon-fallback">OA</span>`;
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8" />
...
<style>
  * { box-sizing: border-box; }
  html, body { width: 100%; height: 100%; margin: 0; overflow: hidden; background: transparent; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif; }
  .reminder { width: 100%; height: 100%; display: grid; grid-template-columns: 38px minmax(0, 1fr) 24px; align-items: center; gap: 10px; padding: 10px 11px; border: 1px solid rgba(255,255,255,.16); border-radius: 15px; color: #f5f5f7; background: rgba(35,36,41,.92); box-shadow: 0 14px 36px rgba(0,0,0,.34); backdrop-filter: blur(28px) saturate(1.15); cursor: default; }
  .app-icon { width: 38px; height: 38px; display: block; border-radius: 10px; object-fit: cover; box-shadow: 0 1px 5px rgba(0,0,0,.32); }
  .app-icon-fallback { display: grid; place-items: center; color: #fff; background: #171b25; font-size: 10px; font-weight: 700; }
  .copy { min-width: 0; display: grid; gap: 3px; }
  .meta { min-width: 0; display: flex; align-items: center; gap: 6px; color: rgba(245,245,247,.68); font-size: 10.5px; line-height: 1.1; }
  .app-name { overflow: hidden; font-weight: 650; text-overflow: ellipsis; white-space: nowrap; }
  .time { color: rgba(245,245,247,.46); font-weight: 500; white-space: nowrap; }
  .title { overflow: hidden; color: rgba(255,255,255,.96); font-size: 13px; font-weight: 630; line-height: 1.25; text-overflow: ellipsis; white-space: nowrap; }
  .close { width: 24px; height: 24px; padding: 0; border: 0; border-radius: 50%; color: rgba(255,255,255,.62); background: rgba(255,255,255,.09); box-shadow: inset 0 0 0 1px rgba(255,255,255,.08); font: 18px/22px -apple-system, BlinkMacSystemFont, sans-serif; cursor: pointer; opacity: .72; }
  .close:hover { color: #fff; background: rgba(255,255,255,.16); opacity: 1; }
</style>
</head>
<body>
  <div class="reminder" role="alert" onclick="location.href='openassist-reminder://open'">
    ${iconHTML}
    <div class="copy">
      <div class="meta"><span class="app-name">OpenAssist</span><span class="time">${escapeHTML(dueText)}</span></div>
      <span class="title">${escapeHTML(payload.title)}</span>
    </div>
    <button type="button" class="close" aria-label="Dismiss" title="Dismiss" onclick="event.stopPropagation(); location.href='openassist-reminder://dismiss'">&times;</button>
  </div>
</body>
</html>`;
}

function clearPlannerReminderAutoHide() {
  if (!plannerReminderAutoHideTimer) return;
  clearTimeout(plannerReminderAutoHideTimer);
  plannerReminderAutoHideTimer = null;
}

function showPlannerReminderFallback(payload: PlannerReminderDuePayload) {
  clearPlannerReminderAutoHide();
  let window = plannerReminderWindow;
  if (!window || window.isDestroyed()) {
    ...
    window.webContents.on("will-navigate", (event, url) => {
      if (!url.startsWith("openassist-reminder://")) return;
      event.preventDefault();
      const shouldOpen = url.startsWith("openassist-reminder://open");
      clearPlannerReminderAutoHide();
      window?.hide();
      if (!shouldOpen) return;
      ensureRegularDockPresence("open planner reminder");
      if (!mainWindow || mainWindow.isDestroyed()) createMainWindow();
      mainWindow?.show();
      mainWindow?.focus();
    });
    window.on("closed", () => {
      if (plannerReminderWindow === window) plannerReminderWindow = null;
      clearPlannerReminderAutoHide();
    });
    plannerReminderWindow = window;
  }
  const display = screen.getDisplayNearestPoint(screen.getCursorScreenPoint());
  const workArea = display.workArea || display.bounds;
  window.setBounds({
    x: Math.round(workArea.x + workArea.width - 360 - 14),
  ...
  void window.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(plannerReminderFallbackHTML(payload))}`);
  plannerReminderAutoHideTimer = setTimeout(() => {
    plannerReminderAutoHideTimer = null;
    plannerReminderWindow?.hide();
  }, 15_000);
  plannerReminderAutoHideTimer.unref?.();
  debugLog(`planner reminder fallback shown day=${payload.dayID} item=${payload.itemID} native=${payload.nativeFailure ?? "unconfirmed"}`);
}
...
function openAssistRepoRoot() {
  const starts = [process.cwd(), app.getAppPath()];
  for (const start of starts) {
    let candidate = path.resolve(start);
    for (let depth = 0; depth < 5; depth += 1) {
      if (fs.existsSync(path.join(candidate, ".git")) || fs.existsSync(path.join(candidate, "companion-projects"))) {
        return candidate;
      }
...
function appleEventKitHelperSourcePath() {
  const candidates = [
    ...
    path.join(openAssistRepoRoot(), "apps", "desktop", "electron", "helpers", "apple-eventkit-helper.swift")
  ];
  return candidates.find((candidate) => fs.existsSync(candidate));
}
...
function appleEventKitHelperInfoPlistPath() {
  const candidates = [
    ...
    path.join(openAssistRepoRoot(), "apps", "desktop", "electron", "helpers", "apple-eventkit-helper-info.plist")
  ];
  return candidates.find((candidate) => fs.existsSync(candidate));
}
...
function whisperHelperSourcePath() {
  const candidates = [
    ...
    path.join(openAssistRepoRoot(), "apps", "desktop", "electron", "helpers", "whisper-transcribe-helper.swift")
  ];
  return candidates.find((candidate) => fs.existsSync(candidate));
}
...
function whisperFrameworkSourcePath() {
  const candidates = [
    ...
    path.join(openAssistRepoRoot(), "apps", "desktop", "electron", "helpers", "whisper.framework"),
    path.join(openAssistRepoRoot(), "legacy", "swift", "Vendor", "Whisper", "whisper.xcframework", "macos-arm64_x86_64", "whisper.framework")
  ];
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("openassist:planner-daily-digest", async (_event, dayID?: string) =>
    (await openAssistBridge()).generatePlannerDailyDigest(dayID)
  );
  ...
  ipcMain.handle("openassist:reset-mac-sync-peer", async (_event, peerID: string) =>
    (await openAssistBridge()).resetMacSyncPeer(peerID)
  );
  ...
  ipcMain.handle("openassist:realtime-stop-delegation", async (_event, taskID?: string) =>
    (await openAssistBridge()).stopCodexRealtimeDelegation(taskID)
  );
  ...
  ipcMain.handle("openassist:live-voice-hud-action", async (_event, action: string) => {
    ...
    if (typeof action === "string" && action.startsWith("openLink:")) {
      const url = action.slice("openLink:".length).trim();
      if (/^https?:\/\/[^\s]+$/i.test(url)) {
        debugLog(`live HUD open link host=${(() => { try { return new URL(url).hostname; } catch { return "invalid"; } })()}`);
  ...
  if (process.platform === "darwin") {
    setTimeout(() => { void ensureAppActivationMonitorHelper().catch(() => undefined); }, 3_000);
  }
...
app.on("will-quit", () => {
  ...
  voiceHUDWindow = null;
  clearPlannerReminderAutoHide();
  plannerReminderWindow?.close();
  plannerReminderWindow = null;
  screenSelectionWindow?.close();
  screenSelectionWindow = null;
